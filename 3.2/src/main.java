...
public class main {
    public static void main(String[] args){
        ...
        System.out.println("--------------------------------------------------------------------");
        System.out.println("| Input the number of interface in a main ring                     |");
        System.out.println("| (This should be less than number of processors and larger than 0)|");
        System.out.println("--------------------------------------------------------------------");
        int N ;
        N = input.nextInt();
        ...
        for (int i = 1; i < 3 * num_processor; i++) {
            arr.add(i);
        }
        Collections.shuffle(arr);

            ArrayList<Integer> arr2 = new ArrayList<Integer>();
            //set the position of interface in random
            ...
            for (int i = 0; i < num_processor; i++) {
                processes.add(new Processor(arr2.get(i)));
            }

            //change the processor's status if it is a interface
            for (int i = 0; i < num_processor; i++) {
                if (processes.get(i).getMyID() == -1) {
                    processes.get(i).setIfInterface(true);
                }
            }

            //tester
            System.out.println("This is the original main ring");
            for (int i = 0; i < num_processor; i++) {
                System.out.print(processes.get(i).getMyID() + " + ");
            }
            System.out.print(processes.get(0).getMyID());
            System.out.println();

            //change interface's myID
            for (int i = 0; i < num_processor; i++) {
                if (processes.get(i).getMyID() == -1) {
                    processes.get(i).generator();
//                    System.out.println("This p has a subring and myID is " + processes.get(i).getMyID());
//                    System.out.println("round is " + processes.get(i).getRound());
                }
            }
        ...
        */
            ...
            System.out.println("This is the main ring after replacing interface with leader of subring");
            for (int i = 0; i < num_processor; i++) {
                System.out.print(processes.get(i).getMyID() + " -> ");
            }
            ...
            //Finally, add those message and rounds in subRing
            for (int i = 0; i < num_processor; i++) {
                if (processes.get(i).getIfInterface()) {
                    round = round + processes.get(i).getRound();
                    message = message + processes.get(i).getMessage();
                }
            }
